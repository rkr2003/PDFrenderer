...
public class CMYKColorSpace extends ColorSpace {
    ...
    /**
     * Converts to CMYK from CIEXYZ. We cheat here, using the RGB colorspace to do the math for us. The toCIEXYZ function has a
     * description of how this is supposed to work, which may be implemented in the future.
     *
    ...
    /**
     * Converts a given RGB to CMYK. RGB doesn't really use black, so K will always be 0. On printers, the black should actually look
     * dark brown. RGB (an additive space) is simply the backwards from CMY (a subtractive space), so all we do is:
     *
     * C = 1-R M = 1-G Y = 1-B
     *
     * @param p_rgbvalue
    ...
    public float[] fromRGB(final float[] p_rgbvalue) {
        ...
        /*
         * TODO: Maybe we should do a better job to determine when black should be used and pulled out? -- At this time, it's not
         * necessary for our (Scantegrity's) uses.
         */
    ...
    /**
     * Converts the CMYK color to CIEXYZ. Because CIEXYZ is 3-component, we cheat, converting to RGB and then using the RGB colorspace
     * function to do the conversion. Details on this colorspace are available on wikipedia:
     *
     * http://en.wikipedia.org/wiki/CIE_XYZ_color_space
     *
     * There is also an "ideal relationship" to CMYK, which might be implemented in the future (don't recall the reference we got this
     * from, probably color.org):
     *
    ...
    public float[] toCIEXYZ(final float[] p_colorvalue) {
        ...
        final float[] l_rgb = this.toRGB(p_colorvalue);
        final ColorSpace l_cs = ColorSpace.getInstance(ColorSpace.TYPE_RGB);
